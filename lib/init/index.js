...
const load = async (cmd) => {
  try {
    ...
    //检查是否存在package.rig.json5.json5
    if (fs.existsSync('package.rig.json5.json5')) {
      console.log(chalk.green('package.rig.json5.json5 already exists~'));
    } else {
      //创建package.rig.json5.json5
      let package.rig.json5Obj = [];
      console.log(chalk.green('create package.rig.json5.json5'));
      fs.writeFileSync('./package.rig.json5.json5', json5.stringify(package.rig.json5Obj));
    }
    //检查rigs是否存在
